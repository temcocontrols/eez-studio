...
export class HomeTab implements IHomeTab {
    ...
    private _homeComponent = React.lazy(() => import("home/home-tab").then(m => ({ default: m.Home })));

    render() {
        const HomeComponent = this._homeComponent;
        return <React.Suspense fallback={null}><HomeComponent /></React.Suspense>;
    }
...
export class ProjectEditorTab implements IHomeTab {
    ...
    async loadProject() {
        try {
            ...
            console.error("loadProject FAILED:", err?.message || err, err?.stack);
            runInAction(() => {
